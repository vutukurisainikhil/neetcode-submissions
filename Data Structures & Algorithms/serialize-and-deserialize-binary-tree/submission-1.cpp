...
public:
    ...
    // queue should be used to decode with NULL logic, in preorder building the
    // tree back

    string serialize(TreeNode* root) {
        if (root == NULL)
            return "null"; // NULL wiil help to decode this logic
        string s;
    ...
    TreeNode* helper_deserialize(queue<string>& q) {
        ...
        q.pop();
        if (s == "null")
            return NULL; // stops adding on left/right.
        int num = stoi(s);
    ...
    TreeNode* deserialize(string data) {
        queue<string> q; // queue should be used
        string temp;
