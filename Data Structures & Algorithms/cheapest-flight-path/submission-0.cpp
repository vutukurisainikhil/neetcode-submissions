...
public:
    int findCheapestPrice(int n, vector<vector<int>>& flights, int src, int dst, int k) {
        ...
        for(auto flight : flights){
            // flight[0] represent node i, flight[1] represent neighbor node of node i,
            // flight[2] represent cost between node i to neighbor node
        ...
        while(!q.empty() && stops <= k){
            int size = q.size();
            while (size--) { // check for all possibilites and then loop again.
