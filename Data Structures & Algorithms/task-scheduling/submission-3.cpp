...
public:
    ...
    int leastInterval(vector<char>& tasks, int n) {
        ...
        // count the no of maxFreq elements.
        int maxFreqTasksCount = count_if(count.begin(), count.end(),
                                    [maxFreq](int c) { return c == maxFreq; });

        // Two candidates for the answer:
        // 1. Skeleton-based lower bound: (maxFreq - 1) * (n + 1) + maxFreqTasksCount
        // 2. Just the total task count (when there's enough variety to never idle)
        int skeleton = (maxFreq - 1) * (n + 1) + maxFreqTasksCount;
