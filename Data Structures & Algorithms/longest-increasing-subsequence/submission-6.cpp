class Solution {
   public:
    ...
    int lengthOfLIS(vector<int>& nums) {
        vector<int> LIS(nums.size(), 1);  // initialize to 1, as each num is sequence.
...
class Solution {
public:
    // TimeComplexity - O(nlogn) as we use loop + binary search O(logn) where as DP LIS takes
O(n^2).
    //  Space Complexity - O(n)

    // 1. create a new array(res), and try to fill elements in sorted way to get max increasing
subsequence
