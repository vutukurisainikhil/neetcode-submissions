   ...
   public:
    string minWindow(string str, string pat) {
        ...
        for (int j = 0; j < len1; j++) {
            ...
            if (count == len2) {
                // removing duplicate elements in hash_str by iterating to s[i] & incrementing start
                // (removed chars are not needed) similarly moving forward to fins minimum window
                // substring
